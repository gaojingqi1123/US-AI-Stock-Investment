...
import { useStockQuote, useMarketSummary } from '@/hooks/useRealtimeStock';
...
export default function Dashboard() {
  ...
  const { quotes: marketQuotes, isFetching: marketFetching } = useMarketSummary()
  const { quotes: stockQuotes, isFetching: stockFetching } = useStockQuote(
    watchlist.map(s => s.symbol)
  )
  const isLive = !marketFetching && !stockFetching
  ...
  const liveWatchlist = watchlist.map(stock => {
    const live = stockQuotes[stock.symbol]
  ...
  const liveMarket = marketOverview.map(m => {
    const idxMap: Record<string, string> = { 'SPY': '^GSPC', 'QQQ': '^IXIC', 'DIA': '^DJI' }
    const live = marketQuotes[idxMap[m.symbol] || '']
    if (live && live.price > 0) {
      return { ...m, price: live.price, change: live.changePercent }
    }
    return m
  })
    ...
    <div className="max-w-[1440px] mx-auto p-4 lg:p-6 space-y-6">
      ...
      <div className="flex items-center justify-between px-1">
        <div className="flex items-center gap-2">
          <span className={`inline-flex items-center gap-1.5 text-xs font-mono font-medium px-2 py-1 rounded-full ${isLive ? 'bg-[#00E676]/10 text-[#00E676]' : 'bg-[#FBBF24]/10 text-[#FBBF24]'}`}>
            <span className={`w-1.5 h-1.5 rounded-full ${isLive ? 'bg-[#00E676] animate-pulse' : 'bg-[#FBBF24]'}`} />
            {isLive ? 'REAL-TIME' : 'SYNCING...'}
          </span>
          <span className="text-[11px] text-text-secondary font-mono">
            Auto-refresh every 5s
          </span>
        </div>
        {(marketFetching || stockFetching) && (
          <RefreshCw className="w-3.5 h-3.5 text-[#00D9C0] animate-spin" />
        )}
      </div>
